-- Add optional project reference to expense line items so admin can
-- filter / total expenses by project for QuickBooks reconciliation
-- (Utsav, 2026-07-16 meeting).

alter table public.expense_line_items
  add column project_id uuid references public.projects(id);

create index on public.expense_line_items (project_id);
  ...
  for v_line in select * from jsonb_array_elements(coalesce(p_lines, '[]'::jsonb)) loop
    ...
    insert into public.expense_line_items
      (expense_id, line_date, category, description, amount_cad, gst_cad,
       credit_card_id, receipt_url, project_id, position)
    values (
      p_expense_id,
