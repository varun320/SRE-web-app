  ...
  return (
    <tr className="border-t border-[var(--color-border-soft)] hover:bg-[var(--color-surface-2)]/40 transition-colors">
      <td className="px-3 py-3"><CategoryCell
        ...
        disabled={disabled}
      /></td>
      <td className="px-3 py-3"><ProjectCell
      ...
      /></td>
      {DAY_KEYS.map((k, i) => (
        <td key={k} className="px-1.5 py-3 text-center">
      ...
      <td className="px-3 py-3">
        {missingProject && <span className="text-xs text-[var(--color-status-declined-fg)]">project required</span>}
